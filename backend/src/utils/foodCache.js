...
export async function getFoodFromCache(fdcId) {
  ...
  const foodNutrients = await db.query(
    'SELECT nutrient_name, value, unit_name FROM food_nutrients WHERE food_id = $1',
    [fdcId]
  );
...
export async function addFoodToCache(food) {
  ...
  for (let nutrient of food.foodNutrients) {
    await db.query(
      'INSERT INTO food_nutrients (food_id, nutrient_name, value, unit_name) VALUES ($1, $2, $3, $4)',
      [food.fdcId, nutrient.nutrientName, nutrient.value, nutrient.unitName]
    );
  }
}
